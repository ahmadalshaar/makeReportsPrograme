from PyQt6.QtWidgets import QGridLayout, QHBoxLayout, QApplication , QListWidgetItem, QListWidget, QToolBar, QSplitter,QWidget,QVBoxLayout,QPushButton,QCheckBox,QLineEdit,QLabel,QMessageBox,QItemDelegate, \
...
from PyQt6.QtGui import QIcon,QFont,QIntValidator,QScreen,QPixmap,QCursor
...
if is_admin():       
    ...
    class ReportEditor(QMainWindow):#QWidget
        ...
        def initUI(self):
            ...
            #button_list_widget.setMinimumWidth(50)
            button_list_widget.setMaximumWidth(200)
            #button_list_layout.addWidget(label_widget)            
            ...
            addButton("addNewreport.png", "إنشاء تقرير", self.createReportFun)
            addButton("savedReports.png", "التقارير المحفوظة", self.savedReportsFun)
            addButton("save.png", "حفظ باسم", self.SavePrograme)

            addButton("clearFields.png", "إفراغ الحقول", self.emptyFieldsFun)
            addButton("summary.png", "ملخص التقارير", self.summaryReports)
            addButton("printer.png", "الطباعة", self.printDoc)
            addButton("control.png", "لوحة التحكم", self.controlPanel)
            addButton("dbExport.png", "تصدير", self.exportDb)
            addButton("importDb.png", "إستيراد", self.importDb)
            addButton("word.png", "word حفظ بصيغة", self.writeWord)
            addButton("pdfIcon.png", "pdf حفظ بصيغة", lambda x, y="Pdf": self.writeWord(y))
            
            
            companyLogobutton = QPushButton()                
            companyLogobutton.setStyleSheet(f"font-size:14px;qproperty-icon:url('companyLogo.png');qproperty-iconSize:150px 100px;background-color:transparent;")   
            companyLogobutton.clicked.connect(self.openwebSite)         
            button_list_layout.addWidget(companyLogobutton) 
            
            mylogo = QLabel()
            MyLogopix = QPixmap("MyLogo.png")
            ...
            self.hiderFrameshow = QFrame(report_widget) #-> old way
            ...
            self.cFrameshow = QFrame(report_widget) ##-> oldway
            # self.cFrame = QFrame()
        ...
        def exportDb(self):
            ...
            if len(filePath)> 0:
                ...
                d = QMessageBox(parent=self,text="تم تصدير قاعدة البيانات بنجاح")
        ...
        def completeImportDb(self):
            fileDbUser = QFileDialog.getOpenFileName(self.windowCreating,"Select a File",desktopPath,filter="Database File (*.db)")
        ...
        def creating(self,fromW):
            ...
            addButton("save.png", "حفظ باسم", self.SavePrograme)
            if fromW != "Local":
                addButton("save.png", "حفظ", self.updateAReport)
            addButton("clearFields.png", "إفراغ الحقول", self.emptyFieldsFun)
            addButton("summary.png", "ملخص التقارير", self.summaryReports)
            addButton("printer.png", "الطباعة", self.printDoc)
            addButton("control.png", "لوحة التحكم", self.controlPanel)
        ...
        def showReport(self, programeNameShow):
            ...
            self.windowshow.setWindowIcon(QIcon("icon.ico"))
        ...
        def putImage(self,ob,number=-1):
            ...
            try:
                ...
                pass
            if ob=="lol":
                responce = QFileDialog.getOpenFileName(self.windowCreating,"Select a File",desktopPath,filter="Image File (*.png *.jpg *.jpeg)")
                if len(responce[0])!=0:
                    image = Image.open(responce[0])
        ...
        def createGoals(self):
            ...
            self.programeGoalsE.setAlignment(Qt.AlignmentFlag.AlignJustify)
        ...
        def load_data(self):
            ...
            for i in cr.fetchall():
                ...
                icon_button.setStyleSheet("Qproperty-icon:url(popUpwindow.png); qproperty-iconSize:30px 30px; background-color:transparent")
